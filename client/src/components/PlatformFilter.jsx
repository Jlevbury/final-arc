import { useCallback, useEffect, useState } from 'react';
import OutlinedInput from '@mui/material/OutlinedInput';
import InputLabel from '@mui/material/InputLabel';
import MenuItem from '@mui/material/MenuItem';
import FormControl from '@mui/material/FormControl';
import ListItemText from '@mui/material/ListItemText';
import Select from '@mui/material/Select';
import Checkbox from '@mui/material/Checkbox';

const KEY = import.meta.env.VITE_RAWG_KEY;

const ITEM_HEIGHT = 48;
const ITEM_PADDING_TOP = 8;
const MenuProps = {
  PaperProps: {
    style: {
      maxHeight: ITEM_HEIGHT * 4.5 + ITEM_PADDING_TOP,
      width: 250,
    },
  },
};

function PlatformFilter({ onSelectPlatform }) {
  const [isLoading, setIsLoading] = useState(false);
  const [, setError] = useState('');
  const [platforms, setPlatforms] = useState([]);

  const fetchPlatforms = useCallback(async () => {
    try {
      setIsLoading(true);
      setError('');
      const res = await fetch(`https://api.rawg.io/api/platforms?key=${KEY}`);

      if (!res.ok)
        throw new Error('Something went wrong with fetching platforms');

      const data = await res.json();
      if (data.count === 0) throw new Error('Genres not found');

      const platformArray = [];

      data.results.forEach(platform => {
        platformArray.push(platform.name);
      });

      setPlatforms(platformArray);
      setIsLoading(false);
      setError('');
    } catch (err) {
      console.log(err);
      if (err.name !== 'AbortError') setError(err.message);
    } finally {
      setIsLoading(false);
    }
  }, [setError, setIsLoading]);

  useEffect(() => {
    fetchPlatforms();
  }, [fetchPlatforms]);

  // const handleChange = e => {
  //   e.preventDefault();

  //   const selectedOptions = Array.from(e.target.selectedOptions, option =>
  //     option.value.toLowerCase()
  //   );

  //   const updatedArray = selectedOptions.join(',');

  //   onSelectPlatform(updatedArray);
  // };

  const handleChange = e => {
    const {
      target: { value },
    } = e;
    setPlatforms(
      // On autofill we get a stringified value.
      typeof value === 'string' ? value.split(',') : value
    );
  };
  console.log(platforms);

  if (isLoading) {
    return;
  }
  return (
    <div>
      <FormControl sx={{ m: 1, width: 300 }}>
        <InputLabel id='demo-multiple-checkbox-label'>Tag</InputLabel>
        <Select
          labelId='demo-multiple-checkbox-label'
          id='demo-multiple-checkbox'
          multiple
          value={platforms}
          onChange={handleChange}
          input={<OutlinedInput label='Tag' />}
          renderValue={selected => selected.join(', ')}
          MenuProps={MenuProps}>
          {platforms?.map((platform, index) => (
            <MenuItem key={index} value={platform}>
              <Checkbox checked={platforms.indexOf(platform) > -1} />
              <ListItemText primary={platform} />
            </MenuItem>
          ))}
        </Select>
      </FormControl>
    </div>
  );
  //   <div>
  //     <label>Select a Platform</label>
  //     <select data-te-select-init multiple className='search' name='genre'>
  //       <option value='all'>All</option>
  //       {platforms?.map((platform, index) => {
  //         return (
  //           <option value={platform} key={index}>
  //             {platform}
  //           </option>
  //         );
  //       })}
  //     </select>
  //   </div>
  // );
}

export default PlatformFilter;
